...
interface WheelState {
  spinning: boolean;
  rotation: number;
  currentSegmentNames: string[];
}

export default function Wheel({ websites, onSelect }: WheelProps) {
  ...
  const { spinning, rotation, currentSegmentNames } = state;
  ...
  useEffect(() => {
    if (websites.length > 0) {
      setState((prevState) => ({
        ...prevState,
        currentSegmentNames: websites.map((site) => {
          try {
            const url = new URL(
  ...
  const spin = () => {
    ...
    const randomOffsetWithinSegment =
      (Math.random() - 0.5) * segmentAngle * 0.8; // Spin to somewhere within the segment, not just center
    ...
    const additionalRotation =
      baseRotation + rotationToAlignStart - randomOffsetWithinSegment;

    setState((prevState) => ({
      ...prevState,
      rotation: prevState.rotation + additionalRotation,
      spinning: true,
    }));
    ...
    websites.length > 0
      ? currentSegmentNames
          .map((_, i) => {
            ...
            return `${color} ${i * segmentAngle}deg ${
              (i + 1) * segmentAngle
            }deg`;
          })
          .join(", ")
      : "transparent 0deg 360deg"; // Fallback for no websites

  return (
    <div className="flex flex-col items-center space-y-6 sm:space-y-8">
      <div className="relative w-60 h-60 sm:w-72 sm:h-72 md:w-80 md:h-80 select-none mt-4">
        {/* Wheel */}
        <div
          ref={wheelRef}
        ...
        {/* Pointer */}
        <div
          className="absolute top-[4px] left-1/2 transform -translate-x-1/2 -translate-y-1/2 rotate-180
         w-0 h-0 
      ...
      <button
        className={`mt-6 sm:mt-8 px-8 py-3 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white rounded-lg font-semibold tracking-wide shadow-md hover:shadow-lg transition-all transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 ${
